...
class EvalVisitor : public Python3ParserBaseVisitor {
    ...
    */
    ...
    virtual std::any visitArglist(Python3Parser::ArglistContext *ctx) override {
        ...
        if (len == 1) {
            std::any tt;
            if(ctx->argument(0)->ASSIGN()) {
                tt = visit(ctx->argument(0)->test(1));
                trans_into_val(tt);
            }
        ...
        for (int i = 0; i < len; i++) {
            if (ctx->argument(i)->ASSIGN()) {
                tmp[i] = visit(ctx->argument(i)->test(1));
                trans_into_val(tmp[i]);
            }
    ...
    virtual std::any visitExpr_stmt(Python3Parser::Expr_stmtContext *ctx) override {
        ...
        for (int i = len - 2; i >= 0; i--) {
            ...
            if (lst) {
                int lenlst = (*lst).size();
                for (int j = 0; j < lenlst; j++) trans_into_val((*lst)[j]);
            }
    ...
    virtual std::any visitTypedargslist(Python3Parser::TypedargslistContext *ctx) override {
        ...
        for(int i = 0; i < len; i++) {
            ...
            if (i + test_len >= len) {
                ...
                std::any ini = visit(ctx->test(test_pos));
                trans_into_val(ini);
                inis.push_back (ini);
